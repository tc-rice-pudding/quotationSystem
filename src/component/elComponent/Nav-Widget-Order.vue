  ...
  <div class="navi-widget clearfix">
    ...
    <ul class="zone-container">
      <li class="zone" :key="zone.rid" v-for="(zone, index) in zones">
        <span class="zone-name" @click.stop="link2Zone(zone)">{{ zone.name }}</span>
        <span
        ...
        <div class="sub-zone" v-show="zone.open">
          <ul class="clearfix" v-if="zone.children && zone.children.length">
            <li class="zone-anchor" v-for="subzone in zone.children" :key="subzone.rid" @click="link2Zone(subzone)">
              {{ subzone.name }}
            </li>
          </ul>
...
import { ref, watch, computed, onMounted, onBeforeUnmount, defineComponent } from 'vue';
import { useRoute, useRouter } from 'vue-router';
import { getSpaceNew } from '@/api/orderDlb';
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    // 获取节点信息：仅返回数据
    const fetchNodeInfo = async (rid: string): Promise<SpaceNodeType | null> => {
      const result = { name: '建行数据中心', resourceId: 'project_root', spaceType: '', path: '', number: 'project_root' };
      if (true) {
        // console.log('result=====', result);
        const { name, resourceId: rid, parentId: pid, path: path, spaceType: spaceType, description, number } = result;
        return { name, rid, pid, path, spaceType, description, open: false, number };
      }
    ...
    // 获取子节点信息：仅返回数据
    const fetchSubNodesInfo = async (rid: string): Promise<SpaceNodeType[] | null> => {
      let _params = { resourceId: rid || 'project_root' };
      const result = (await getSpaceNew(_params)) as unknown as ResType;
      if (result && `${result.status}` === '200' && result.data) {
        if (rid === 'project_root') {
          sortFn(result?.data);
        }
        const { data } = result;
        return data.map((item: { name: string; path: string; parentId: string; description: string; resourceId: string; spaceType: string }) => {
          const { name, path = '', description, parentId: pid, resourceId: rid, spaceType: spaceType, number } = item;
          return {
            name,
            rid,
            pid,
    ...
    const setSubNodesData = async (rid: string) => {
      ...
      if (data && data.length) {
        ...
        data.forEach(obj => {
          ...
          if (obj.description !== '库房') {
            dataWithRoute.push(obj);
          }
        });
    ...
    const link2Zone = async (zone: SpaceNodeType) => {
      ...
      if (zone) {
        const { routes } = props;
        const { pid, spaceType } = zone;
        zone.route = routes[spaceType];
        const index = zones.value.findIndex(i => i.rid === pid); // 父节点的位置
        // if (index > -1) {
        zones.value = zones.value.slice(0, index + 1).concat(zone);
        setSubNodesData(zone.rid);
        // if (zone.rid === 'project_root') {
        //   return;
        // } else {
        context.emit('pathLocation', zones.value[zones.value.length - 1]);
        // }
    ...
    const setInitNavbar = async (rid: string) => {
      ...
      if (nodeData) {
        ...
        // if (
        //   +space_type >= props.startTab &&
        //   (!props.endTab || +space_type <= props.endTab)
        // ) {
        setSubNodesData(rid);
